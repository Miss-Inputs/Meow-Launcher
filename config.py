...
emulator_configs = [
	...
	#Use -C useBios=0 for homebrew with bad checksum/logo that won't boot on real hardware. Some intensive games (e.g. Doom) will not run at full speed on toaster, but generally it's fine
	...
	#Often pretty slow but okay for turn-based games; environment variable is needed for GLideN64 which sometimes is preferred over Rice and sometimes not (the latter wins at speed and not much else). Do I still need that environment variable? I think I might
	...
	#Mednafen assumes that there is only 1 gamepad and it's the 6 button kind, so button mapping is kind of weird when I was perfectly fine just using 2 buttons
	...
	#Analog stuff like Gorf doesn't really work that well, but it doesn't in real life either; could use -sio casette -cass *.wav if there was ever a game that came as a .wav which apparently could be a thing in theory
	...
	#Can also use bndarc for Bandai version but that doesn't seem to make any difference at all? Some games seem to be weird with the input so that sucks
	...
	#I wonder if there's a way to set the overlay programmatically... doesn't look like there's a command line option to do that. Also the buttons are kinda wack I must admit, as they're actually a horizontal row of 4
	...
	#You may notice there is no command_line here, we'll deal with that later (as we're using MAME, so we need to make sure we're using NTSC or PAL as necessary)
	...
	#"But why not just use Amstrad CPC+?" you ask, well, there's no games that are on CPC+ cartridges that aren't on GX4000, and I don't feel like fondling around with disks and tapes
	...
	#This is interesting, because this runs alright on toasters, but Neo Geo-based arcade games do not (both being emulated in MAME); meaning I am probably doing something wrong. Don't think it has region lock so I should never need to use neocdzj?
	...
	#Controls are actually fine in-game, just requires a keypad to select levels/start games and that's not consistent at all so good luck with that. All carts are either USA or combination USA/Europe, so why play in 50Hz when we don't have to
	...
	#There's a keypad there which is used for game selection/setup, otherwise it just uses a paddle with a button (the actual controllers IRL were wacky, but for emulation purposes are otherwise pretty normal). Hopefully adding that RAM expansion won't hurt? Some games (Chicken) seem to be broken anyway whoops
	...
	#Sorta has like... 2 sets of A and B buttons, and 3 buttons on one side and 2 on the other? It's supposed to be ambidextrous or something which is cool in real life but not so great here, I might need to look more into it and then maybe move it into the normal-but-less-cool platforms
	...
	#Oof this is just super mega weird because you can turn the thing sideways and it still does a thing. I'll need some point of reference to figure out how to set this up for a normal-ish gamepad...
	...
	#Yay! I got a computer working! I don't fully understand disks (they present me with a menu with Japanese text I can't read), so carts will have to do, luckily there are quite a number of cartridge-based games
	...
	#Still don't understand disks. This includes MSX2+ but Turbo-R doesn't work in MAME, and I'm just presuming this Panasonic thing is the best MSX2+ system to emulate because I looked it up and it sounds cool
	...
	#Need to figure out which region to use and we can only really do that by filename, also it doesn't like 16KB carts; disks and tapes are a pain in the ass IRL so MAME emulates the ass-pain of course; and I dunno about .prg files, those are just weird (but it'd be great if I could get those working though); with this and C64 there are some games where you'll have to manually change to the paddle which kinda sucks but I guess it can't be helped and also turn on "Paddle Reverse" in "Analog Controls" for some reason
	...
	#Might get a bit of slowdown on toaster if you open the MAME menu, but usually you'd want to do that when paused anyway, and the games themselves run fine
...
if not is_toaster:
	emulator_configs.extend([
		...
		#Seems like some PAL games don't run at the resolution Mednafen thinks they should, so they need per-game configs that override the scanline start/end settings
		...
		#Will not run full screen from the command line and you always have to set it manually whether you like it or not (I do not)
		...
		#Snes9x's GTK+ port doesn't let us load carts with slots for other carts from the command line yet, so this will have to do, but unfortunately it's a tad slower
		...
		#More similar to the C64 than the VIC-20, need to plug a joystick into both ports because once again games can use either port and thanks I hate it. At least there's only one TV type
		...
		#Hey!! We finally have floppies working!! Because they boot automatically! Assumes that they will all work fine though without any other disks, and this will need to be updated if we see any cartridges (MAME says it has a cart slot)...
