...
class MednafenModule(EmulatorInfo):
	def __init__(self, module, supported_extensions, params=None):
		...
		EmulatorInfo.__init__(self, params, supported_extensions, ['zip', 'gz'])

class MameDriver(EmulatorInfo):
	def __init__(self, launch_params, supported_extensions):
		EmulatorInfo.__init__(self, launch_params, supported_extensions, ['7z', 'zip'])
...
class ViceEmulator(EmulatorInfo):
	def __init__(self, params):
		...
		EmulatorInfo.__init__(self, params, ['d64', 'g64', 'x64', 'p64', 'd71', 'd81', 'd80', 'd82', 'd1m', 'd2m'] + ['20', '40', '60', '70', '80', 'a0', 'b0', 'e0', 'crt', 'bin'] + ['p00', 'prg', 'tap', 't64'], [])

emulators = {
	'A7800': EmulatorInfo(command_lines.a7800, ['bin', 'a78'], ['7z', 'zip']),
	...
	'cxNES': EmulatorInfo(command_lines.cxnes, ['nes', 'fds', 'unf', 'unif'], ['7z', 'zip']),
	'Dolphin': EmulatorInfo(command_lines.dolphin, ['iso', 'gcm', 'gcz', 'tgc', 'elf', 'dol', 'wad', 'wbfs', 'm3u'], []),
	'Flycast': EmulatorInfo(command_lines.flycast, ['gdi', 'cdi', 'chd', 'cue'], []),
	'FS-UAE': EmulatorInfo(command_lines.fs_uae, ['iso', 'cue', 'adf', 'ipf'], []),
	#Note that .ipf files need a separately downloadable plugin. We could detect the presence of that, I guess
	'Gambatte': EmulatorInfo(command_lines.gambatte, ['gb', 'gbc'], ['zip']),
	...
	'GBE+': EmulatorInfo(command_lines.gbe_plus, ['gb', 'gbc', 'gba'], []),
	#Also in theory recognizes any extension and assumes Game Boy if not .gba or .nds, but that would be screwy
	'Kega Fusion': EmulatorInfo(command_lines.kega_fusion, ['bin', 'gen', 'md', 'smd', 'sgd', 'gg', 'sms', 'iso', 'cue', 'sg', 'sc', '32x'], ['zip']),
	...
	'mGBA': EmulatorInfo(command_lines.mgba, ['gb', 'gbc', 'gba', 'srl', 'bin', 'mb'], ['7z', 'zip']),
	'Mupen64Plus': EmulatorInfo(command_lines.mupen64plus, ['z64', 'v64', 'n64'], []),
	'PCSX2': EmulatorInfo(LaunchParams('PCSX2', ['--nogui', '--fullscreen', '--fullboot', '$<path>']), ['iso', 'cso', 'bin'], ['gz']),
	...
	'PokeMini': EmulatorInfo(LaunchParams('PokeMini', ['-fullscreen', '$<path>']), ['min'], ['zip']),
	...
	'PokeMini (wrapper)': EmulatorInfo(MultiCommandLaunchParams([LaunchParams('mkdir', ['-p', os.path.expanduser('~/.config/PokeMini')]), LaunchParams('cd', [os.path.expanduser('~/.config/PokeMini')]), LaunchParams('PokeMini', ['-fullscreen', '$<path>'])]), ['min'], ['zip']),
	'PPSSPP': EmulatorInfo(command_lines.ppsspp, ['iso', 'pbp', 'cso'], []),
	'Reicast': EmulatorInfo(command_lines.reicast, ['gdi', 'cdi', 'chd'], []),
	'SimCoupe': EmulatorInfo(LaunchParams('simcoupe', ['-fullscreen', 'yes', '$<path>']), ['mgt', 'sad', 'dsk', 'sbt'], ['zip', 'gz']),
	...
	'MAME (Amstrad GX4000)': MameDriver(command_lines.mame_system('gx4000', 'cart'), ['bin', 'cpr']),
	...
	'MAME (Astrocade)': MameDriver(command_lines.mame_system('astrocde', 'cart', {'exp': 'rl64_ram'}), ['bin']),
	...
	'MAME (Atari 2600)': MameDriver(command_lines.mame_atari_2600, ['bin', 'a26']),
	...
	'MAME (Atari 5200)': MameDriver(command_lines.mame_system('a5200', 'cart'), ['bin', 'rom', 'car', 'a52']),
	...
	'MAME (Coleco Adam)': MameDriver(command_lines.mame_coleco_adam, ['wav', 'ddp'] + mame_floppy_formats),
	#Both disks and tapes autoboot. Woohoo!
	'MAME (Entex Adventure Vision)': MameDriver(command_lines.mame_system('advision', 'cart'), ['bin']),
	#Doesn't work with the "Code Red" demo last time I tried
	'MAME (FM-7)': MameDriver(command_lines.mame_system('fm77av', 'flop1'), mame_floppy_formats),
	...
	'MAME (Gamate)': MameDriver(command_lines.mame_system('gamate', 'cart'), ['bin']),
	'MAME (Game Boy)': MameDriver(command_lines.mame_game_boy, ['bin', 'gb', 'gbc']),
	...
	'MAME (Game.com)': MameDriver(command_lines.mame_system('gamecom', 'cart1'), ['bin', 'tgc']),
	...
	'MAME (Game Gear)': MameDriver(command_lines.mame_game_gear, ['bin', 'gg']),
	'MAME (Epoch Game Pocket Computer)': MameDriver(command_lines.mame_system('gamepock', 'cart'), ['bin']),
	'MAME (GBA)': MameDriver(command_lines.mame_system('gba', 'cart'), ['bin', 'gba']),
	#Does not let you do GBA-enhanced GBC games
	'MAME (IBM PCjr)': MameDriver(command_lines.mame_ibm_pcjr, mame_floppy_formats + ['img', 'bin', 'jrc']),
	'MAME (Intellivision)': MameDriver(command_lines.mame_intellivision, ['bin', 'int', 'rom', 'itv']),
	'MAME (Lynx)': MameDriver(command_lines.mame_lynx, ['lnx', 'lyx', 'o']),
	#Could be weird where rotation is involved
	'MAME (Master System)': MameDriver(command_lines.mame_master_system, ['bin', 'sms']),
	'MAME (Mattel Juice Box)': MameDriver(command_lines.mame_system('juicebox', 'memcard'), ['smc']),
	'MAME (Mega Drive)': MameDriver(command_lines.mame_megadrive, ['bin', 'md', 'smd', 'gen']),
	'MAME (Mega Duck)': MameDriver(command_lines.mame_system('megaduck', 'cart'), ['bin']),
	'MAME (Memorex VIS)': MameDriver(command_lines.mame_system('vis', 'cdrom'), mame_cdrom_formats),
	'MAME (MSX)': MameDriver(command_lines.mame_msx1, ['bin', 'rom'] + mame_floppy_formats),
	'MAME (MSX2)': MameDriver(command_lines.mame_msx2, ['bin', 'rom'] + mame_floppy_formats),
	'MAME (Neo Geo CD)': MameDriver(command_lines.mame_system('neocdz', 'cdrom'), mame_cdrom_formats),
	...
	'MAME (Neo Geo Pocket)': MameDriver(command_lines.mame_system('ngpc', 'cart'), ['bin', 'ngp', 'npc', 'ngc']),
	'MAME (NES)': MameDriver(command_lines.mame_nes, ['nes', 'unf', 'unif', 'fds']),
	...
	'MAME (Vectrex)': MameDriver(command_lines.mame_system('vectrex', 'cart'), ['bin', 'gam', 'vec']),
	#Includes overlays as selectable artwork, but that has to be done by the user from the menu
	'MAME (VIC-10)': MameDriver(command_lines.mame_system('vic10', 'cart', {'joy1': 'joy', 'joy2': 'joy'}, has_keyboard=True), ['crt', '80', 'e0']),
	...
	'MAME (VIC-20)': MameDriver(command_lines.mame_vic_20, ['20', '40', '60', '70', 'a0', 'b0', 'crt']),
	'MAME (V.Smile)': MameDriver(command_lines.mame_system('vsmile', 'cart'), ['u1', 'u3', 'bin']),
	'MAME (VZ-200)': MameDriver(command_lines.mame_system('vz200', 'dump', {'io': 'joystick', 'mem': 'laser_64k'}, True), ['vz']),
	...
	'MAME (Watara Supervision)': MameDriver(command_lines.mame_system('svision', 'cart'), ['bin', 'ws', 'sv']),
	...
	'MAME (CreatiVision)': MameDriver(command_lines.mame_system('crvision', 'cart'), ['bin', 'rom']),
	...
	'MAME (GameKing)': MameDriver(command_lines.mame_system('gameking', 'cart'), ['bin']),
	'MAME (GameKing 3)': MameDriver(command_lines.mame_system('gamekin3', 'cart'), ['bin']),
	...
	'MAME (Mattel Aquarius)': MameDriver(command_lines.mame_system('aquarius', 'cart'), ['bin', 'rom']),
	#Controllers aren't emulated yet (and they're necessary for a lot of things)
	'MAME (Mega CD)': MameDriver(command_lines.mame_mega_cd, mame_cdrom_formats),
	#Hmm sometimes works and sometimes does not
	'MAME (Microtan 65)': MameDriver(command_lines.mame_system('mt65', 'dump'), ['dmp', 'm65']),
	...
	'MAME (SAM Coupe)': MameDriver(command_lines.mame_system('samcoupe', 'flop1', autoboot_script='sam_coupe'), mame_floppy_formats),
	...
	'MAME (VideoBrain)': MameDriver(command_lines.mame_system('vidbrain', 'cart'), ['bin']),
	...
	'MAME (Bandai RX-78)': MameDriver(command_lines.mame_system('rx78', 'cart'), ['bin', 'rom']),
	...
	'MAME (LeapPad)': MameDriver(command_lines.mame_system('leappad', 'cart'), ['bin']),
