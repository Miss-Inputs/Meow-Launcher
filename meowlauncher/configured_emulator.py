...
class ConfiguredEmulator(ConfiguredRunner):
	...
	def get_launch_command_for_game(
		self, game: 'EmulatedGame', platform_config_options: 'Mapping[str, TypeOfConfigValue]'
	) -> 'LaunchCommand':
		assert (
			self.runner.launch_command_func
		), 'launch_command_func should never have been left as None'
...
class LibretroCoreWithFrontend(ConfiguredStandardEmulator):
	def __init__(
		...
		core_as_emulator = StandardEmulator(
			display_name,
			core.status,
			'',
			core.launch_command_func,
			core.supported_extensions,
			frontend.supported_compression,
			host_platform=frontend.host_platform,
		)
	...
	def get_launch_command_for_game(
		...
		return self.frontend.launch_command_func(
			game, platform_config_options, self.core_config, self.frontend_config
		)
