...
_standalone_emulators: 'Collection[StandardEmulator]' = {
	StandardEmulator(
		'A7800', EmulatorStatus.Good, 'a7800', command_lines.a7800, {'bin', 'a78'}, {'7z', 'zip'}
	),
	...
	StandardEmulator(
		...
		EmulatorStatus.Good,
		'duckstation-qt',
		command_lines.duckstation,
		{'bin', 'img', 'cue', 'chd', 'exe', 'm3u', 'iso'},
		configs={
			'compatibility_xml_path': RunnerConfigValue(
				Path, None, 'Path to where compatibility.xml is installed'
			),  # Because DuckStation's not always installed in any particular location…
			'gamedb_path': RunnerConfigValue(Path, None, 'Path to where gamedb.json is installed'),
			'compatibility_threshold': RunnerConfigValue(
				int, 2, "Don't try and launch any game with this compatibility rating or lower"
			),
			'consider_unknown_games_incompatible': RunnerConfigValue(
				bool,
				False,
	...
	StandardEmulator(
		...
		EmulatorStatus.Good,
		'flycast',
		simple_emulator(['-config', 'window:fullscreen=yes', rom_path_argument]),
		{'gdi', 'cdi', 'chd', 'cue'},
		[],
	),
	StandardEmulator(
	...
	StandardEmulator(
		'GBE+', EmulatorStatus.Good, 'gbe_plus_qt', command_lines.gbe_plus, {'gb', 'gbc', 'gba'}
	),
	...
	StandardEmulator(
		'melonDS', EmulatorStatus.Good, 'melonDS', command_lines.melonds, {'nds', 'srl'}
	),  # Supports .dsi too, but I'm acting as though it doesn't, because it's too screwy
	StandardEmulator(
		'Mupen64Plus',
		EmulatorStatus.Good,
		'mupen64plus',
		command_lines.mupen64plus,
		{'z64', 'v64', 'n64'},
	),
	StandardEmulator(
		'PCSX2',
	...
	StandardEmulator(
		...
		EmulatorStatus.Good,
		'pico8',
		simple_emulator(['-windowed', '0', '-run', rom_path_argument]),
		{'p8', 'p8.png'},
	),
	StandardEmulator(
		'PokeMini', EmulatorStatus.Good, 'PokeMini', command_lines.pokemini, {'min'}, {'zip'}
	),  # Normally just puts the config files in the current directory, so this cd's to ~/.config/PokeMini first
	StandardEmulator(
		'PPSSPP', EmulatorStatus.Good, 'ppsspp-qt', command_lines.ppsspp, {'iso', 'pbp', 'cso', '/'}
	),
	StandardEmulator(
		'Reicast', EmulatorStatus.Good, 'reicast', command_lines.reicast, {'gdi', 'cdi', 'chd'}
	),
	...
	StandardEmulator(
		...
		EmulatorStatus.Good,
		'simcoupe',
		simple_emulator(['-fullscreen', 'yes', rom_path_argument]),
		{'mgt', 'sad', 'dsk', 'sbt'},
		{'zip', 'gz'},
	),
	StandardEmulator(
	...
	StandardEmulator(
		...
		EmulatorStatus.Good,
		'stella',
		simple_emulator(['-fullscreen', '1', rom_path_argument]),
		{'a26', 'bin', 'rom'}.union(atari_2600_cartridge_extensions),
		{'gz', 'zip'},
	),
	StandardEmulator(
		'PrBoom+', EmulatorStatus.Imperfect, 'prboom-plus', command_lines.prboom_plus, {'wad'}
	),
	...
	StandardEmulator(
		'Xemu', EmulatorStatus.Experimental, 'xemu', command_lines.xemu, {'iso'}
	...
	MednafenModule(
		...
		command_lines.mednafen_apple_ii,
	),
	...
	SimpleMednafenModule(
		'PC Engine', EmulatorStatus.Good, 'pce', {'pce', 'sgx', 'iso', 'cue', 'ccd', 'toc', 'm3u'}
	),
	SimpleMednafenModule(
		'PC-FX', EmulatorStatus.Good, 'pcfx', {'iso', 'cue', 'toc', 'ccd', 'm3u'}
	),  # Do NOT specify a FX-SCSI BIOS
	SimpleMednafenModule(
		'PlayStation', EmulatorStatus.Good, 'psx', {'iso', 'cue', 'exe', 'toc', 'ccd', 'm3u', 'psx'}
	),
	...
	MAMEDriver(
		...
		simple_mame_driver('gx4000', 'cart'),
		{'bin', 'cpr'},
	),
	...
	MAMEDriver(
		'Apple IIgs',
		EmulatorStatus.Good,
		simple_mame_driver('apple2gsr1', 'flop3', {'gameio': 'joy'}, has_keyboard=True),
	...
	MAMEDriver(
		'Apple III',
		EmulatorStatus.Good,
		simple_mame_driver('apple3', 'flop1', has_keyboard=True),
		mame_floppy_formats.union({'do', 'po'}),
	),
	...
	MAMEDriver(
		'Astrocade',
		EmulatorStatus.Good,
		simple_mame_driver('astrocde', 'cart', {'exp': 'rl64_ram'}),
		{'bin'},
	),
	...
	MAMEDriver(
		...
		EmulatorStatus.Good,
		simple_mame_driver('sv8000', 'cart'),
		{'bin'},
	),
	MAMEDriver(
		'BBC Bridge Companion', EmulatorStatus.Good, simple_mame_driver('bbcbc', 'cart'), {'bin'}
	),
	MAMEDriver('Casio PV-1000', EmulatorStatus.Good, simple_mame_driver('pv1000', 'cart'), {'bin'}),
	MAMEDriver(
		'Champion 2711',
		EmulatorStatus.Good,
		simple_mame_driver('unichamp', 'cart'),
		generic_cart_extensions,
	),
	MAMEDriver(
		'Channel F', EmulatorStatus.Good, simple_mame_driver('channelf', 'cart'), {'bin', 'chf'}
	),
	MAMEDriver(
		'ColecoVision', EmulatorStatus.Good, command_lines.mame_colecovision, {'bin', 'col', 'rom'}
	),
	# MT06554: Roller controller is inaccurate
	MAMEDriver(
		'Coleco Adam',
		EmulatorStatus.Good,
		command_lines.mame_coleco_adam,
		{'wav', 'ddp'}.union(mame_floppy_formats),
	),
	# Both disks and tapes autoboot. Woohoo!
	MAMEDriver(
		'Entex Adventure Vision',
		EmulatorStatus.Imperfect,
		simple_mame_driver('advision', 'cart'),
		{'bin'},
	),
	MAMEDriver(
		'FM-7',
		EmulatorStatus.Good,
		simple_mame_driver('fm77av', 'flop1', has_keyboard=True),
		mame_floppy_formats,
	),
	...
	MAMEDriver(
		...
		{'bin', 'gb', 'gbc'},
		{
			...
			'prefer_sgb_over_gbc': RunnerConfigValue(
				bool,
				False,
	...
	MAMEDriver(
		'Game.com', EmulatorStatus.Good, simple_mame_driver('gamecom', 'cart1'), {'bin', 'tgc'}
	),
	...
	MAMEDriver(
		...
		EmulatorStatus.Good,
		simple_mame_driver('gamepock', 'cart'),
		{'bin'},
	),
	...
	MAMEDriver(
		'IBM PCjr',
		EmulatorStatus.Good,
		command_lines.mame_ibm_pcjr,
		mame_floppy_formats.union({'img', 'bin', 'jrc'}),
	),
	MAMEDriver(
		'Intellivision',
		EmulatorStatus.Good,
		command_lines.mame_intellivision,
		{'bin', 'int', 'rom', 'itv'},
	),
	MAMEDriver(
		'Jupiter Ace',
		EmulatorStatus.Good,
		simple_mame_driver('jupace', 'dump', {'ramsize': '48K'}, has_keyboard=True),
		{'ace'},
	),
	MAMEDriver('Lynx', EmulatorStatus.Good, command_lines.mame_lynx, {'lnx', 'lyx', 'o'}),
	# Could be weird where rotation is involved
	MAMEDriver(
		'Magnavox Odyssey²', EmulatorStatus.Good, command_lines.mame_odyssey2, {'bin', 'rom'}
	),
	MAMEDriver(
		'Master System', EmulatorStatus.Good, command_lines.mame_master_system, {'bin', 'sms'}
	),
	MAMEDriver(
		'Mattel Juice Box', EmulatorStatus.Good, simple_mame_driver('juicebox', 'memcard'), {'smc'}
	),
	MAMEDriver(
		'Mega Drive', EmulatorStatus.Good, command_lines.mame_megadrive, {'bin', 'md', 'smd', 'gen'}
	),
	MAMEDriver('Mega Duck', EmulatorStatus.Good, simple_mame_driver('megaduck', 'cart'), {'bin'}),
	MAMEDriver(
		'Memorex VIS', EmulatorStatus.Good, simple_mame_driver('vis', 'cdrom'), mame_cdrom_formats
	),
	MAMEDriver(
		'MSX',
		EmulatorStatus.Good,
		command_lines.mame_msx1,
		generic_cart_extensions.union(mame_floppy_formats),
	),
	MAMEDriver(
		'MSX2',
		EmulatorStatus.Good,
		command_lines.mame_msx2,
		generic_cart_extensions.union(mame_floppy_formats),
	),
	MAMEDriver(
		'MSX2+',
		EmulatorStatus.Good,
		command_lines.mame_msx2plus,
		generic_cart_extensions.union(mame_floppy_formats),
	),
	MAMEDriver(
		'Neo Geo CD', EmulatorStatus.Good, simple_mame_driver('neocdz', 'cdrom'), mame_cdrom_formats
	),
	...
	MAMEDriver(
		'Nichibutsu My Vision', EmulatorStatus.Good, simple_mame_driver('myvision', 'cart'), {'bin'}
	),
	MAMEDriver(
		'PC Engine', EmulatorStatus.Good, command_lines.mame_pc_engine, {'pce', 'bin', 'sgx'}
	),
	MAMEDriver(
		'SAM Coupé',
		EmulatorStatus.Good,
		simple_mame_driver('samcoupe', 'flop1', autoboot_script='sam_coupe', has_keyboard=True),
		mame_floppy_formats,
	),
	MAMEDriver(
		'SG-1000',
	...
	MAMEDriver(
		'Sharp X1',
		EmulatorStatus.Good,
		simple_mame_driver('x1turbo40', 'flop1', has_keyboard=True),
	...
	MAMEDriver(
		'SNES', EmulatorStatus.Good, command_lines.mame_snes, {'sfc', 'bs', 'st', 'smc', 'swc'}
	),
	MAMEDriver('Sord M5', EmulatorStatus.Good, command_lines.mame_sord_m5, {'bin'}),
	MAMEDriver(
		'Super Cassette Vision',
		EmulatorStatus.Good,
		command_lines.mame_super_cassette_vision,
		{'bin'},
	),
	MAMEDriver(
		'SVI-3x8', EmulatorStatus.Good, simple_mame_driver('svi328', 'cart'), {'bin', 'rom'}
	),
	MAMEDriver(
		'Squale',
		EmulatorStatus.Good,
		simple_mame_driver('squale', 'cart', has_keyboard=True),
		{'bin'},
	),
	MAMEDriver(
		'Tandy CoCo',
		EmulatorStatus.Good,
		simple_mame_driver('coco3', 'cart', has_keyboard=True),
		{'ccc', 'rom', 'bin'},
	),
	...
	MAMEDriver(
		'Thomson MO5',
		EmulatorStatus.Good,
		simple_mame_driver('mo5', 'flop1', has_keyboard=True),
		{'fd', 'sap'}.union(mame_floppy_formats),
	),
	...
	MAMEDriver(
		'Tomy Tutor',
		EmulatorStatus.Good,
		simple_mame_driver('tutor', 'cart', has_keyboard=True, autoboot_script='tomy_tutor'),
		{'bin'},
	),
	# There is pyuuta if you want to read Japanese instead
	MAMEDriver(
		'VC 4000', EmulatorStatus.Good, simple_mame_driver('vc4000', 'cart'), {'bin', 'rom'}
	),
	...
	MAMEDriver(
		'Vectrex', EmulatorStatus.Good, simple_mame_driver('vectrex', 'cart'), {'bin', 'gam', 'vec'}
	),
	# Includes overlays as selectable artwork, but that has to be done by the user from the menu
	MAMEDriver(
		'VIC-10',
		EmulatorStatus.Good,
		simple_mame_driver('vic10', 'cart', {'joy1': 'joy', 'joy2': 'joy'}, has_keyboard=True),
		{'crt', '80', 'e0'},
	),
	...
	MAMEDriver(
		'VIC-20',
		EmulatorStatus.Good,
		command_lines.mame_vic_20,
		{'20', '40', '60', '70', 'a0', 'b0', 'crt'},
	),
	MAMEDriver(
		'V.Smile',
		EmulatorStatus.Good,
		simple_mame_driver('vsmile', 'cart'),
		generic_cart_extensions,
	),
	MAMEDriver(
		'V.Smile Motion',
		EmulatorStatus.Good,
		simple_mame_driver('vsmilem', 'cart'),
		generic_cart_extensions,
	),
	...
	MAMEDriver(
		'VZ-200',
		EmulatorStatus.Good,
		simple_mame_driver('vz200', 'dump', {'io': 'joystick', 'mem': 'laser_64k'}, True),
		{'vz'},
	),
	...
	MAMEDriver(
		'Watara Supervision',
		EmulatorStatus.Good,
		simple_mame_driver('svision', 'cart'),
		{'bin', 'ws', 'sv'},
	),
	...
	MAMEDriver(
		'WonderSwan',
		EmulatorStatus.Good,
		simple_mame_driver('wscolor', 'cart'),
		{'ws', 'wsc', 'bin', 'pc2'},
	),
	...
	# Based off an old version of VisualBoyAdvance
	SimpleMednafenModule(
		'Master System', EmulatorStatus.ExperimentalButSeemsOkay, 'sms', {'sms', 'bin'}
	),
	...
	SimpleMednafenModule(
		'Saturn', EmulatorStatus.ExperimentalButSeemsOkay, 'ss', {'cue', 'toc', 'ccd', 'm3u'}
	),
	# Doesn't do .iso for whatever strange reason, which is a bit unfortunate. Might do .bin executables? Probably not
	SimpleMednafenModule(
		'SNES', EmulatorStatus.ExperimentalButSeemsOkay, 'snes', {'sfc', 'smc', 'swc'}
	),
	...
	MAMEDriver(
		'32X', EmulatorStatus.ExperimentalButSeemsOkay, command_lines.mame_32x, {'32x', 'bin'}
	),
	...
	MAMEDriver(
		'Bandai RX-78',
		EmulatorStatus.ExperimentalButSeemsOkay,
		simple_mame_driver('rx78', 'cart', has_keyboard=True),
		{'bin', 'rom'},
	),  # Supports savestate but otherwise emulation = preliminary
	MAMEDriver(
		'Casio PV-2000',
		EmulatorStatus.ExperimentalButSeemsOkay,
		simple_mame_driver('pv2000', 'cart', has_keyboard=True),
		{'bin'},
	),
	...
	MAMEDriver(
		'CD-i',
		EmulatorStatus.ExperimentalButSeemsOkay,
		simple_mame_driver('cdimono1', 'cdrom'),
		mame_cdrom_formats,
	...
	MAMEDriver(
		'Dreamcast VMU',
		EmulatorStatus.ExperimentalButSeemsOkay,
		simple_mame_driver('svmu', 'quik', {'bios': 'dev1004'}),
		{'vms', 'bin'},
	),
	...
	MAMEDriver(
		...
		EmulatorStatus.ExperimentalButSeemsOkay,
		simple_mame_driver('gcslottv', 'cart'),
		generic_cart_extensions,
	),
	...
	MAMEDriver(
		'GameKing',
		EmulatorStatus.ExperimentalButSeemsOkay,
		simple_mame_driver('gameking', 'cart'),
		{'bin', 'gk'},
	),
	MAMEDriver(
		'GameKing 3',
		EmulatorStatus.ExperimentalButSeemsOkay,
		simple_mame_driver('gamekin3', 'cart'),
		{'bin', 'gk3'},
	),
	MAMEDriver(
		'GP32', EmulatorStatus.ExperimentalButSeemsOkay, simple_mame_driver('gp32', 'memc'), {'smc'}
	),
	# Bad performance (60-ish% on i5-9600kf) but otherwise might kinda work?
	MAMEDriver(
		'Hartung Game Master',
		EmulatorStatus.ExperimentalButSeemsOkay,
		simple_mame_driver('gmaster', 'cart'),
		{'bin'},
	),
	...
	MAMEDriver(
		'PC-6001',
		EmulatorStatus.ExperimentalButSeemsOkay,
		simple_mame_driver('pc6001', 'cart1', has_keyboard=True),
		{'bin', 'rom'},
	),
	...
	MAMEDriver(
		'PC-88',
		EmulatorStatus.ExperimentalButSeemsOkay,
		simple_mame_driver('pc8801', 'flop1', has_keyboard=True),
		mame_floppy_formats,
	),
	...
	MAMEDriver(
		'Sharp MZ-2000',
		EmulatorStatus.ExperimentalButSeemsOkay,
		simple_mame_driver('mz2200', 'flop1', has_keyboard=True),
		mame_floppy_formats.union({'2d'}),
	),
	...
	MAMEDriver(
		'Sony SMC-777',
		EmulatorStatus.ExperimentalButSeemsOkay,
		simple_mame_driver('smc777', 'flop1', has_keyboard=True),
		mame_floppy_formats.union({'1dd'}),
	),
	MAMEDriver(
		'Uzebox',
		EmulatorStatus.ExperimentalButSeemsOkay,
		simple_mame_driver('uzebox', 'cart'),
		{'bin', 'uze'},
	),
	MAMEDriver(
		'V.Tech Socrates',
		EmulatorStatus.ExperimentalButSeemsOkay,
		simple_mame_driver('socrates', 'cart'),
		{'bin'},
	),
	...
	MAMEDriver(
		...
		simple_mame_driver('crvision', 'cart', has_keyboard=True),
		{'bin', 'rom'},
	),
	...
	MAMEDriver(
		'IBM PC',
		EmulatorStatus.Janky,
		simple_mame_driver('ibm5150', 'flop1', {'isa5': 'sblaster1_5'}, has_keyboard=True),
	...
	MAMEDriver(
		'Mattel Aquarius',
		EmulatorStatus.Experimental,
		simple_mame_driver('aquarius', 'cart', has_keyboard=True),
		{'bin', 'rom'},
	),
	...
	MAMEDriver(
		'Mattel HyperScan',
		EmulatorStatus.Experimental,
		simple_mame_driver('hyprscan', 'cdrom'),
		mame_cdrom_formats,
	),
	...
	MAMEDriver(
		'Microtan 65',
		EmulatorStatus.Experimental,
		simple_mame_driver('mt65', 'dump', has_keyboard=True),
		{'dmp', 'm65'},
	),
	...
	MAMEDriver(
		'Microvision',
		EmulatorStatus.Janky,
		simple_mame_driver('microvsn', 'cart'),
		generic_cart_extensions,
	),
	...
	MAMEDriver(
		'Pokémon Mini',
		EmulatorStatus.Experimental,
		simple_mame_driver('pokemini', 'cart'),
		{'bin', 'min'},
	),
	...
	MAMEDriver(
		"Super A'Can", EmulatorStatus.Experimental, simple_mame_driver('supracan', 'cart'), {'bin'}
	),
	# Some things work, except with no sound, so... nah
	MAMEDriver(
		'TRS-80',
		EmulatorStatus.Experimental,
		simple_mame_driver('trs80l2', 'quik', has_keyboard=True),
		{'cmd'},
	),
	...
	MAMEDriver(
		'V.Smile Baby',
		EmulatorStatus.Experimental,
		simple_mame_driver('vsmileb', 'cart'),
		{'u1', 'u3', 'bin'},
	),
	...
	MAMEDriver(
		'VideoBrain',
		EmulatorStatus.Experimental,
		simple_mame_driver('vidbrain', 'cart', has_keyboard=True),
		{'bin'},
	),
	...
	MAMEDriver(
		'Videoton TVC',
		EmulatorStatus.Experimental,
		simple_mame_driver('tvc64', 'cart', has_keyboard=True),
		{'bin', 'rom', 'crt'},
	),
	MAMEDriver(
		'Virtual Boy',
		EmulatorStatus.Experimental,
		simple_mame_driver('vboy', 'cart'),
		{'bin', 'vb'},
	),
	...
	MAMEDriver(
		'3DO', EmulatorStatus.Borked, simple_mame_driver('3do', 'cdrom'), mame_cdrom_formats
	),  # Should switch to 3do_pal when needed, but it doesn't really matter at this point
	MAMEDriver(
		'Buzztime Home Trivia System',
		EmulatorStatus.Borked,
		simple_mame_driver('buzztime', 'cart'),
		{'bin'},
	),
	...
	MAMEDriver(
		'Casio Loopy', EmulatorStatus.Borked, simple_mame_driver('casloopy', 'cart'), {'bin'}
	),
	# Just shows corrupted graphics (and has no controls defined), basically just a skeleton even if it looks like it isn't
	MAMEDriver(
		'Commodore CDTV',
		EmulatorStatus.Borked,
		simple_mame_driver('cdtv', 'cdrom'),
		mame_cdrom_formats,
	),
	# This one works less than CD32; just takes you to the default boot screen like no CD was inserted
	MAMEDriver(
		'Copera', EmulatorStatus.Borked, simple_mame_driver('copera', 'cart'), {'bin', 'md'}
	),
	# Displays the logo and then displays nothing
	MAMEDriver(
		'GoGo TV Video Vision', EmulatorStatus.Borked, simple_mame_driver('tvgogo', 'cart'), {'bin'}
	),
	MAMEDriver(
		'Jaguar CD',
		EmulatorStatus.Borked,
		simple_mame_driver('jaguarcd', 'cdrom'),
		mame_cdrom_formats,
	...
	MAMEDriver(
		'Advanced Pico Beena', EmulatorStatus.Borked, simple_mame_driver('beena', 'cart'), {'bin'}
	),  # Segfaults
	...
	MAMEDriver(
		'Leapster', EmulatorStatus.Borked, simple_mame_driver('leapster', 'cart'), {'bin'}
	...
	MAMEDriver(
		'Monon Color', EmulatorStatus.Borked, simple_mame_driver('mononcol', 'cart'), {'bin'}
	),
	MAMEDriver(
		'My First LeapPad', EmulatorStatus.Borked, simple_mame_driver('mfleappad', 'cart'), {'bin'}
	),
	MAMEDriver(
		'Pippin', EmulatorStatus.Borked, simple_mame_driver('pippin', 'cdrom'), mame_cdrom_formats
	),
	MAMEDriver(
		'Pocket Challenge W',
		EmulatorStatus.Borked,
		simple_mame_driver('pockchal', 'cart'),
		{'bin', 'pcw'},
	),
	...
	MAMEDriver(
		'V.Smile Pro',
		EmulatorStatus.Borked,
		simple_mame_driver('vsmilpro', 'cdrom'),
		mame_cdrom_formats,
	),
...
_libretro_cores = {
	...
	LibretroCore(
		'Beetle NeoPop', EmulatorStatus.Good, 'mednafen_ngp', None, {'ngp', 'ngc', 'ngpc', 'npc'}
	),
	LibretroCore(
		'Beetle PCE Fast',
		EmulatorStatus.Good,
	...
	LibretroCore(
		'ChaiLove', EmulatorStatus.Good, 'chailove', None, {'chai', 'chailove'}
	),  # needs_fullpath=true
	LibretroCore('Dinothawr', EmulatorStatus.Good, 'dinothawr', None, {'game'}),
	LibretroCore(
		'fMSX', EmulatorStatus.Good, 'fmsx', None, {'rom', 'mx1', 'mx2', 'dsk', 'm3u', 'cas', 'fdi'}
	),
	...
	LibretroCore(
		'FUSE', EmulatorStatus.Good, 'fuse', None, {'tzx', 'tap', 'z80', 'rzx', 'scl', 'trd', 'dsk'}
	),
	LibretroCore(
		'Gearboy',
		EmulatorStatus.Good,
	...
	LibretroCore(
		'Hatari', EmulatorStatus.Good, 'hatari', None, {'st', 'msa', 'stx', 'dim', 'm3u'}
	...
	LibretroCore(
		'Mesen', EmulatorStatus.Good, 'mesen', command_lines.mesen, {'nes', 'fds', 'unf', 'unif'}
	),
	LibretroCore(
		'melonDS', EmulatorStatus.Good, 'melonds', command_lines.melonds, {'nds'}
	...
	LibretroCore(
		'ProSystem', EmulatorStatus.Good, 'prosystem', command_lines.prosystem, {'a78', 'bin'}
	),
	LibretroCore(
		'PUAE',
		EmulatorStatus.Good,
...
emulators = {
	standalone_emulator.config_name: standalone_emulator
	for standalone_emulator in _standalone_emulators
}
...
_libretro_frontends = {
	LibretroFrontend(
		'RetroArch', EmulatorStatus.Good, 'retroarch', command_lines.retroarch, {'7z', 'zip'}
	)
}
