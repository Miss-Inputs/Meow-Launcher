...
class StandardEmulator(Emulator):
	...
	def __init__(self, display_name: str, status: EmulatorStatus, default_exe_name: str, launch_command_func: Optional[LaunchCommandFunc], supported_extensions: Sequence[str], supported_compression: Sequence[str]=None, configs: Mapping[str, RunnerConfigValue]=None, host_platform=HostPlatform.Native):
		super().__init__(display_name, status, default_exe_name, launch_command_func, configs, host_platform)
...
class MednafenModule(StandardEmulator):
	def __init__(self, status: EmulatorStatus, supported_extensions: Sequence[str], params_func: LaunchCommandFunc, configs: Mapping[str, RunnerConfigValue]=None):
...
class MAMEDriver(StandardEmulator):
	def __init__(self, status: EmulatorStatus, launch_params: LaunchCommandFunc, supported_extensions: list[str], configs: Optional[MutableMapping[str, RunnerConfigValue]]=None):
		if configs is None:
			configs = {}
		configs.update({
...
class ViceEmulator(StandardEmulator):
	def __init__(self, status: EmulatorStatus, default_exe_name: str, params: LaunchCommandFunc):
...
class LibretroCore(Emulator):
	def __init__(self, name: str, status: EmulatorStatus, default_exe_name: str, launch_command_func: Optional[LaunchCommandFunc], supported_extensions: Sequence[str], configs: Optional[dict[str, RunnerConfigValue]]=None):
		...
		super().__init__(name, status, default_path, launch_command_func, configs=configs)
