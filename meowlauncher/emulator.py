...
class Emulator(Runner, Generic[EmulatorGameType_co]):
	...
	def __init__(self, name: str, status: EmulatorStatus, default_exe_name: str, launch_command_func: 'GenericLaunchCommandFunc[EmulatorGameType_co] | None', host_platform: HostPlatform=HostPlatform.Linux, config_name: str|None=None):
...
class StandardEmulator(Emulator['ROMGame']):
	...
	def __init__(self, display_name: str, status: EmulatorStatus, default_exe_name: str, launch_command_func: 'ROMGameLaunchFunc', supported_extensions: 'Collection[str]', supported_compression: 'Collection[str] | None'=None, host_platform: HostPlatform=HostPlatform.Linux, config_name: str | None=None):
		super().__init__(display_name, status, default_exe_name, launch_command_func, host_platform, config_name)
...
class MednafenModule(StandardEmulator):
	def __init__(self, name: str, status: EmulatorStatus, supported_extensions: 'Collection[str]', params_func: 'ROMGameLaunchFunc'):
		StandardEmulator.__init__(self, 'Mednafen', status, 'mednafen', params_func, supported_extensions, {'zip', 'gz'}, config_name=f'Mednafen ({name})')
...
class LibretroCore(Emulator['EmulatedGame']):
	def __init__(self, name: str, status: EmulatorStatus, default_exe_name: str, launch_command_func: 'GenericLaunchCommandFunc[EmulatedGame] | None', supported_extensions: 'Collection[str]'):
		self.supported_extensions = supported_extensions
		default_path = main_config.libretro_cores_directory.joinpath(default_exe_name + '_libretro.so').resolve()
		super().__init__(name, status, str(default_path), launch_command_func, config_name=name + ' (libretro)')
...
class LibretroFrontend(Runner):
	def __init__(self, name: str, status: EmulatorStatus, default_exe_name: str, launch_command_func: 'LibretroFrontendLaunchCommandFunc', supported_compression: 'Collection[str] | None'=None, host_platform: HostPlatform=HostPlatform.Linux):
		self._name = name
		self.status = status
		self.default_exe_name = default_exe_name
		self.launch_command_func = launch_command_func
