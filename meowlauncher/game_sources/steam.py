		...
		if launch_item_config:
			...
			if oslist:
				...
				if osarch:
					platform += '_' + (
						str(osarch.data)
						if isinstance(osarch, appinfo.Integer)
						else osarch.decode('utf-8', errors='backslashreplace')
					)
...
def add_genre(game: 'SteamGame', common: Mapping[bytes, Any]) -> None:
	...
	if additional_genre_ids:
		game.info.specific_info['Additional Genres'] = tuple(
...
def add_info_from_appinfo_common_section(game: 'SteamGame', common: Mapping[bytes, Any]) -> None:
	...
	if original_release_timestamp and steam_release_timestamp:
		...
		game.info.specific_info['Steam Release Date'] = Date(
			steam_release_datetime.year, steam_release_datetime.month, steam_release_datetime.day
		)
	...
	if store_asset_mtime:
		...
		game.info.specific_info['Store Asset Modification Time'] = Date(
			store_asset_timestamp.year, store_asset_timestamp.month, store_asset_timestamp.day
		)
	...
	if store_categories_list:
		...
		cats = {
			store_categories.get(key, key)
			for key in (
				key.decode('utf-8', errors='backslashreplace')
				for key in store_categories_list.keys()
			)
		}
		...
		for cat in cats:
			if (
				'multiplayer' in cat.lower()
				or 'multi-player' in cat.lower()
				or 'co-op' in cat.lower()
				or 'split screen' in cat.lower()
			):
	...
	has_adult_content = common.get(
		b'has_adult_content'
	)  # Integer object with data = 0 or 1, as most bools here seem to be
	...
	if metacritic_name:
		game.info.add_alternate_name(
			metacritic_name.decode('utf8', 'backslashreplace'), 'Metacritic Name'
		)
	...
	game.info.specific_info['Controlller Support'] = common.get(
		b'controller_support', b'none'
	).decode('utf-8', errors='backslashreplace')

	if game.steam_installation.localization:
		store_tag_names = game.steam_installation.localization['localization']['english'][
			'store_tags'
		]
		store_tag_ids_list = common.get(b'store_tags')
		if store_tag_ids_list:
			store_tags = {
				store_tag_names.get(id, id)
				for id in (str(value.data) for value in store_tag_ids_list.values())
			}
	...
	if associations:
		...
		for i, association in associations.items():
			...
			if not association_type or not association_name_bytes:
				continue
			association_name = (
				str(association_name_bytes.data)
				if isinstance(association_name_bytes, appinfo.Integer)
				else association_name_bytes.decode('utf-8', 'backslashreplace')
			)
			if association_type == b'franchise':
				not_actual_franchises = {
					'Playism',
					'Hentai',
					'Coming-of-Age',
					'Wolf RPG Editor',
		...
		game.info.series = [
			franchise
			for _, franchise in sorted(franchises.items())
			if franchise not in publishers.values()
		]

		game.info.specific_info['Mac Developer'] = [
			dev.removesuffix(' (Mac)')
			for _, dev in sorted(developers.items())
			if dev.endswith(' (Mac)')
		]
		game.info.specific_info['Linux Developer'] = [
			dev.removesuffix(' (Linux)')
			for _, dev in sorted(developers.items())
			if dev.endswith(' (Linux)')
		]
		game.info.developer = [
			dev for _, dev in sorted(developers.items()) if not dev.endswith((' (Mac)', ' (Linux)'))
		]

		game.info.specific_info['Mac Publisher'] = [
			pub.removesuffix(' (Mac)')
			for _, pub in sorted(publishers.items())
			if pub.endswith(' (Mac)')
		]
		game.info.specific_info['Linux Publisher'] = [
			pub.removesuffix(' (Linux)')
			for _, pub in sorted(publishers.items())
			if pub.endswith(' (Linux)')
		]
		game.info.publisher = [
			game.info.developer[0] if pub == 'Self Published' and game.info.developer else pub
			for _, pub in sorted(publishers.items())
			if not pub.endswith((' (Mac)', ' (Linux)'))
		]


def add_info_from_appinfo_extended_section(
	game: 'SteamGame', extended: Mapping[bytes, Any]
) -> None:
	if not game.info.developer:
		...
		if developer:
			if isinstance(developer, appinfo.Integer):
				...
				game.info.developer = normalize_developer(
					developer.decode('utf-8', errors='backslashreplace')
				)
	if not game.info.publisher:
		...
		if publisher:
			if isinstance(publisher, appinfo.Integer):
				...
				publisher = normalize_developer(
					publisher.decode('utf-8', errors='backslashreplace')
				)
	...
	if developer_url:
		game.info.documents['Developer Homepage'] = developer_url.decode(
...
def process_appinfo_config_section(
	game: 'SteamGame', app_info_section: Mapping[bytes, Any]
) -> None:
...
def add_images(game: 'SteamGame') -> None:
	...
	for image_filename, name in (
		('icon', 'Icon'),
		('header', 'Header'),
		('library_600x900', 'Cover'),
		('library_hero', 'Hero'),
		('logo', 'Logo'),
	):
...
def add_info_from_cache_json(game: 'SteamGame', json_path: Path, is_single_user: bool) -> None:
	...
	if is_single_user and achievements:
		...
		if total_achievements:
			unachieved_list = {
				cheevo['strID']: cheevo for cheevo in achievements.get('vecUnachieved', ())
			}
			achieved_list = {
				cheevo['strID']: cheevo
				for cheevo in achievements.get('vecAchievedHidden', ())
				+ achievements.get('vecHighlight', ())
			}
			if achievement_map:
				for achievement_id, achievement_data in achievement_map[0][1]:
					if achievement_data.get('bAchieved'):
						achieved_list[achievement_id] = dict(
							achieved_list.get(achievement_id, {}), **achievement_data
						)
					else:
						unachieved_list[achievement_id] = dict(
							unachieved_list.get(achievement_id, {}), **achievement_data
						)

			if unachieved_list:
				unachieved_stats = (
					cheevo.get('flAchieved', 0) / 100 for cheevo in unachieved_list.values()
				)
				unachieved_percent = statistics.median(unachieved_stats)
				game.info.specific_info[
					'Median Global Unachieved Completion'
				] = f'{unachieved_percent:.0%}'
			if achieved_list:
				achievement_stats = (
					cheevo.get('flAchieved', 0) / 100 for cheevo in achieved_list.values()
				)
				achieved_percent = statistics.median(achievement_stats)
				game.info.specific_info[
					'Median Global Achieved Completion'
				] = f'{achieved_percent:.0%}'

			game.info.specific_info[
				'Achievement Completion'
			] = f'{achieved / total_achievements:.0%}'
