...
from .engine_info import (
	add_gamemaker_metadata,
...
def _try_detect_unity(
	folder: Path, game_info: 'GameInfo | None', executable: 'Path | None'
) -> str | None:
	...
	if executable and executable.suffix != '.sh':
		...
		for f in folder.iterdir():
			...
			if f.name.endswith('_Data'):
				...
				if (
					f.joinpath('Managed', 'UnityEngine.dll').is_file()
					or f.joinpath('Resources', 'unity default resources').is_file()
				):
					unity_data_folder = f
			...
			with default_resources_path.open('rb') as default_resources:
				...
				try:
					...
					if not 0x30 < version_bytes[0] < 0x3A:
						raise ValueError(
							'Megan is too lazy to refactor this function with proper flow'
						)
...
def _try_detect_ue4(folder: Path, game_info: 'GameInfo | None') -> bool:
	...
	engine_folder = folder.joinpath(
		'Engine'
	)  # I guess this is always there… dunno if anything is _always_ in there though (Binaries + Content?)
	...
	if redist_folder.is_dir():
		for subdir in redist_folder.iterdir():
			...
			if (
				subdir.joinpath('UE4PrereqSetup_x64.exe').is_file()
				or subdir.joinpath('UE4PrereqSetup_x86.exe').is_file()
			):
				return True
	...
	if (
		binaries_folder.joinpath('Linux', project_name + '-Linux-Shipping').is_file()
		or binaries_folder.joinpath('Linux', project_name).is_file()
		or binaries_folder.joinpath('Win64', project_name + '-Win64-Shipping.exe').is_file()
	):
		if game_info:
...
def _try_detect_build(folder: Path) -> bool:
	...
	if 'build.exe' in files and 'bsetup.exe' in files and 'editart.exe' in files:
		return True
	for f in folder.iterdir():
		if f.name.lower() == 'build' and f.is_dir() and _try_detect_build(f):
			return True
	return False
...
def _try_detect_ue3(folder: Path) -> bool:
	for f in folder.iterdir():
		if f.name in {'Game', 'GAME'}:
			continue
		if (
			f.name.endswith('Game')
			or f.name == 'P13'
			or (f.name.isupper() and f.name.endswith('GAME'))
		):
			...
			if f.is_dir():
				if f.joinpath('CookedPC', 'Engine.u').is_file():
					return True
				if (
					f.joinpath('CookedPCConsole').is_dir()
					or f.joinpath('CookedPCConsole_FR').is_dir()
					or f.joinpath('CookedPCConsoleFinal').is_dir()
				):
					return True
				if f.joinpath('CookedWiiU', 'Engine.xxx').is_file():
					return True
				if f.joinpath(
					'COOKEDPS3', 'ENGINE.XXX'
				).is_file():  # PS3 filesystems are in yelling case
					return True
	return False
...
def _try_detect_gamemaker(folder: Path, game_info: 'GameInfo | None') -> bool:
	possible_data_file_paths = [
		folder / 'data.win',
		folder / 'game.unx',
		folder.joinpath('assets', 'data.win'),
		folder.joinpath('assets', 'game.unx'),
	]
...
def _try_detect_nw(folder: Path, game_info: 'GameInfo | None') -> str | None:
	if (
		not folder.joinpath('nw.pak').is_file()
		and not folder.joinpath('nw_100_percent.pak').is_file()
		and not folder.joinpath('nw_200_percent.pak').is_file()
	):
		return None
	...
	if package_json_path.is_file():
		...
		try:
			with zipfile.ZipFile(package_nw_path) as package_nw:
				if (
					'c2runtime.js' in package_nw.namelist()
					or 'scripts/c2runtime.js' in package_nw.namelist()
				):
					subengine = 'Construct 2'
				elif (
					'c3runtime.js' in package_nw.namelist()
					or 'scripts/c3runtime.js' in package_nw.namelist()
				):
		...
		have_package = True
		if game_info and not add_info_from_package_json_zip(package_nw_path, game_info):
			return None
...
def _try_detect_rpg_maker_200x(
	folder: Path, game_info: 'GameInfo | None', executable: Path | None
) -> str | None:
		...
		if game_info:
			try:
				...
				if 'FullPackageFlag' in rpg_rt_ini['RPG_RT']:
					game_info.specific_info['Uses RTP?'] = (
						rpg_rt_ini['RPG_RT']['FullPackageFlag'] == '0'
					)
			except KeyError:
				pass
			titles_path = (
				folder / 'Title' / 'titles.png'
			)  # Sometimes this exists and title.png is a black screen
			...
			with init.open('rt', encoding='utf-8') as f:
				for line in f:
					...
					if version_tuple_match:
						version_with_dots = '.'.join(
							version_tuple_match[1].replace(',', '').split()
						)
...
def _try_detect_rpg_maker_xp_vx(
	folder: Path, game_info: 'GameInfo | None', executable: Path | None
) -> str | None:
	engine_versions = {
		'rgss1': 'RPG Maker XP',
	...
	if engine:
		...
		if game_ini_path.is_file():  # Should be?
			...
			try:
				...
				game = game_ini['Game']
				library = (
					game['Library'].lower().removeprefix('system\\')
				)  # Might not always be in that folder
				...
				if game_info:
					...
					if 'Play_Sound_Effects' in game:
						game_info.specific_info['Has Sound Effects?'] = (
							game['Play_Sound_Effects'] == '1'
						)
					if 'Mouse' in game:
...
def _try_detect_cryengine(folder: Path) -> str | None:
	...
	if info[0] and info[0].get('ProductName') == 'CryEngine2':
...
def _try_detect_jackbox_games(folder: Path, game_info: 'GameInfo | None') -> bool:
	...
	if folder.joinpath('platform.swf').is_file() and jbg_config_jet_path.is_file():
		if game_info:
			...
			game_info.specific_info['Supports Fullscreen?'] = (
				jbg_config_jet.get('supportsFullScreen', 'true') != 'false'
			)
			...
			if supports_gamepad_hotplug:
				game_info.specific_info['Supports Hotplugging Gamepads?'] = (
					supports_gamepad_hotplug != 'false'
				)
		return True
	return False
...
def _try_detect_piko_mednafen(folder: Path, game_info: 'GameInfo | None') -> str | None:
	...
	if data_path.is_file() and (game_path.is_file() or game_cue_path.is_file()):
		...
		with game_path.open('rb') as game:
			...
			if (
				magic
				== b'\x78\x54\xA9\xFF\x53\x01\xAD\x00\x10\x29\x40\xF0\x0C\xA9\x90\x53\x04\x4C\x00\x40 NEC '
			):
...
def _try_detect_engines_from_filenames(folder: Path) -> str | None:
	...
	if 'sierra.exe' in files and any(f.endswith('.scr') for f in files):
		return 'SCI'
	if ('visplayer' in files and any(f.endswith('.vis') for f in files)) or (
		'data.vis' in files
	):  # .vis magic is "VIS3"?
	...
	if folder.joinpath('Pack', 'data').is_dir() and any(
		f.suffix.lower() == '.ttarch' for f in folder.joinpath('Pack', 'data').iterdir()
	):
		return 'Telltale Tool'
	if (
		folder.joinpath('bin', 'libUnigine_x64.so').is_file()
		or folder.joinpath('bin', 'libUnigine_x86.so').is_file()
		or folder.joinpath('bin', 'Unigine_x86.dll').is_file()
		or folder.joinpath('bin', 'Unigine_x64.dll').is_file()
	):
		return 'Unigine'
...
def try_detect_engine_from_exe_properties(
	exe_path: Path, game_info: 'GameInfo | None'
) -> str | None:
...
def try_and_detect_engine_from_folder(
	folder: Path, game_info: 'GameInfo | None' = None, executable: Path | None = None
) -> str | None:
	...
	nw_version = _try_detect_nw(
		folder, game_info
	)  # Not really the right name for this variable, it's to check if it's just nw.js or has RPG Maker MV/MZ inside
	if nw_version:
		return nw_version
