...
def _butler_configure(
	folder: Path, os_filter: str | None = None, *, ignore_arch: bool = False
...
def _is_probably_unwanted_candidate(path: Path, all_candidate_basenames: 'Collection[str]') -> bool:
	...
	if len(all_candidate_basenames) == 2 and name == 'nwjc':
		return True
	if len(all_candidate_basenames) == 3 and (
		'nw' in all_candidate_basenames
		and 'nwjc' in all_candidate_basenames
		and name in {'nw', 'nwjc'}
	):
		return True
	if len(all_candidate_basenames) > 1 and name in {
...
class ItchGame(Game):
	...
	def _add_metadata_from_receipt(self) -> None:
		...
		if created_at:
			...
			self.info.specific_info['Creation Date'] = Date(
		...
		if upload:
			...
			if self.is_demo and 'demo' not in self.name.lower():
				self._name += ' (Demo)'
			self.info.specific_info['Upload Type'] = upload.get(
				'type', 'default'
			)  # default, flash, unity, java, html, soundtrack, book, video, documentation, mod, audio_assets, graphical_assets, sourcecode, other
			self.platforms = set(
				upload.get('platforms', {}).keys()
			)  # I think the values show if it's x86/x64 but eh
			...
			if upload_created_at:
				...
				self.info.specific_info['Upload Creation Date'] = Date(
					upload_creation_date.year, upload_creation_date.month, upload_creation_date.day
				)
			if upload_updated_at:
				upload_date = datetime.date.fromisoformat(upload_updated_at[:10])
				self.info.specific_info['Upload Date'] = Date(
	...
	def _try_and_find_exe(
		self, os_filter: str | None = None, *, no_arch_filter: bool = False
	) -> 'Iterator[tuple[str | None, Path, Mapping[str, bool] | None]]':
		...
		"""
		output = _butler_configure(self.path, os_filter, ignore_arch=no_arch_filter)
		if not output:
			# Bugger
			return
		...
		for candidate in candidates:
			yield (
				candidate['flavor'],
				Path(output['value']['basePath'], candidate['path']),
				candidate.get('windowsInfo'),
			)
		return

	def _make_exe_launcher(
		self, flavour: str | None, exe_path: Path, windows_info: 'Mapping[str, bool] | None'
	) -> None:
		...
		if not params:
			if flavour not in {'app-macos', 'macos'}:
				logger.debug(
					'Not dealing with %s %s %s %s %s at this point in time',
					self.path,
					flavour,
					exe_path,
					self.platforms,
...
def get_launch_params(
	flavour: str, exe_path: Path, windows_info: 'Mapping[str, bool] | None'
) -> tuple[LaunchCommand, str | None] | None:
