...
from meowlauncher.games.specific_behaviour.info_helpers import (
	arcade_machine_finders,
	custom_info_funcs,
	filename_tag_info_funcs,
	rom_file_info_funcs,
	software_info_funcs,
	static_info_funcs,
)
from meowlauncher.info import Date
from meowlauncher.util.detect_things_from_filename import get_tv_system_from_filename_tags
from meowlauncher.util.region_info import (
...
def _add_metadata_from_regions(game_info: 'GameInfo') -> None:
	if game_info.regions and not game_info.languages:
...
def _add_alternate_names(rom: ROM, game_info: 'GameInfo') -> None:
	...
	not_allowed_to_be_primary_name = {
...
def _add_metadata_from_libretro_database_entry(
	game_info: 'GameInfo', database: LibretroDatabaseType, key: str | int
) -> bool:
	database_entry = cast(
		dict[str, Any] | None, database.get(key)
		...
		if database:
			if game.platform.dat_uses_serial and ', ' in cast(str, key):
				for product_code in cast(str, key).split(', '):
					if _add_metadata_from_libretro_database_entry(
						game.info, database, product_code
					):
						break
			else:
	...
	if arcade_equivalent_finder:
		if software:
			...
			if not equivalent_arcade and software.parent_name:
				...
				assert (
					software_parent
				), 'This is impossible, software.parent_name is set but software list has no parent'
