...
class ScummVMGame(Game):
	...
	def __init__(self, game_id: str, scummvm: 'ScummVM', section: Mapping[str, str]):
		super().__init__()
		...
		# TODO: Could potentially define a pydantic schema for the ini section if that helps
	...
	@property
	def name(self) -> str:
		name = self.section.get('description', self.game_id)
		return name.replace(
			'/', ') ('
		)  # Names are usually something like Cool Game (CD/DOS/English); we convert it to Cool Game (CD) (DOS) (English) to make it work better with disambiguate etc
	...
	def engine(self) -> str | None:
		...
		engine_id = self.section.get('engineid')
		if engine_id:
			return self.scummvm.engine_list.get(engine_id, engine_id)
		return None

	@property
	def path(self) -> Path | None:
		"""Path to folder containing game files (or filename in case of Glk etc)"""
		pathstr = self.section.get('path')
		if not pathstr:
			return None
		path = Path(pathstr)  # path
		filename = self.section.get(
			'filename'
		)  # Glk has this, as it is just one file; stops us looking for icons in a folder where we shouldn't etc
		return path / filename if filename else path

	@property
	def language(self) -> Language | None:
		"""Language specified by the "language" option"""
		language_code = self.section.get('language')
	...
	def original_platform(self) -> str | None:
		...
		platform = self.section.get('platform')
		if not platform:
			return None
		if platform == 'amiga' and self.section.get('extra') == 'CD32':
			return 'Amiga CD32'
		return format_platform(platform)

	def add_metadata(self) -> None:
		self.info.input_info.add_option(
			[input_info.Mouse(), input_info.Keyboard()]
		)  # Can use gamepad if you enable it, but I guess to add that as input_info I'd have to know exactly how many buttons and sticks etc it uses
		self.info.save_type = (
			SaveType.Internal
		)  # Saves to your own dang computer so I guess that counts
		self.info.categories = ('Games',)  # Safe to assume this by default
		if self.section.get('gameid') == 'agi-fanmade':
		...
		extra = self.section.get('extra')
		if extra:
			self.info.specific_info[
				'Version'
			] = extra  # Hmm, I guess that'd be how we should use this properly…
		...
		if self.scummvm.config.use_original_platform:
...
class ScummVMLauncher(Launcher):
	...
	def __init__(self, game: ScummVMGame, runner: 'ScummVM') -> None:
