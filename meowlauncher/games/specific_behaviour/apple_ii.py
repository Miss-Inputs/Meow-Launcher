...
def _parse_woz_kv(game_info: GameInfo, key: str, value: str, object_for_warning: Any=None) -> None:
	...
	if key == 'version':
		...
		game_info.languages = {lang for lang in (get_language_by_english_name(lang_name) for lang_name in value.split('|')) if lang}
	elif key == 'genre':
...
def _parse_woz_meta_chunk(game_info: GameInfo, chunk_data: bytes, object_for_warning: Any=None) -> None:
