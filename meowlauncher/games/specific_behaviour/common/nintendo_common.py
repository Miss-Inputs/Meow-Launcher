...
class NintendoAgeRatings:
	...
	def all_present_ratings(self) -> Collection[int | AgeRatingStatus]:
		return [
			b if status == AgeRatingStatus.Present else status
			for b, status in ((b, self._get_rating_status(b)) for b in self.bytes)
			if status != AgeRatingStatus.Missing
		]
...
def add_info_from_local_titles(
	metadata: 'GameInfo',
	short_titles: Mapping[str, str],
	long_titles: Mapping[str, str],
	publishers: Mapping[str, str | None],
	region_codes: Collection[WiiU3DSRegionCode],
) -> None:
	local_short_title = None
	local_long_title = None
	local_publisher: str | None = None
	if (
		WiiU3DSRegionCode.RegionFree in region_codes
		or WiiU3DSRegionCode.USA in region_codes
		or WiiU3DSRegionCode.Europe in region_codes
	):
