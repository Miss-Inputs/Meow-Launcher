...
def _add_banner_title_metadata(
	metadata: 'GameInfo', banner_title: str, language: str | None = None
) -> None:
...
def _parse_banner(
	rom: FileROM, metadata: 'GameInfo', header: bytes, is_dsi: bool, banner_offset: int
) -> None:
	...
	if version in {1, 2, 3, 0x103}:
		...
		for i in range(7):
			try:
				banner_title = (
					banner[0x240 + (i * 256) : 0x240 + (i * 256) + 256]
					.rstrip(b'\0 \xff')
					.decode('utf-16le')
				)
...
def _add_ds_input_info(metadata: 'GameInfo') -> None:
	...
	builtin_buttons.face_buttons = (
		4  # I forgot why we're not counting Start and Select but I guess that's a thing
	)
