...
def parse_gamecube_banner_text(
	game_info: GameInfo, banner_bytes: bytes, encoding: str, lang: str | None = None
) -> None:
...
def add_banner_info(rom: ROM, game_info: GameInfo, banner: bytes) -> None:
	...
	if banner_magic in {b'BNR1', b'BNR2'}:
		...
		encoding = (
			'shift_jis'
			if game_info.specific_info['Region Code'] == NintendoDiscRegion.NTSC_J
			else 'latin-1'
		)
		...
		if banner_magic == b'BNR2':
			...
			for i, lang_name in languages.items():
				...
				parse_gamecube_banner_text(
					game_info, banner[offset : offset + 0x140], encoding, lang_name
				)
...
def add_fst_info(
	rom: FileROM, game_info: GameInfo, fst_offset: int, fst_size: int, offset: int = 0
) -> None:
	if fst_offset and fst_size and fst_size < (128 * 1024 * 1024):
		...
		if fst_size < (number_of_fst_entries * 12):
			logger.info(
				'Invalid FST in %s: size is %s but expected to be at least %s',
				rom,
				format_byte_size(fst_size),
				format_byte_size(number_of_fst_entries * 12),
			)
			return
		...
		for i in range(1, number_of_fst_entries):
			...
			banner_name = string_table[
				offset_into_string_table : offset_into_string_table + len('opening.bnr')
			]
...
def add_apploader_date(header: bytes, game_info: GameInfo) -> None:
	try:
		...
		guessed_release_date = Date(year, month, day, is_guessed=True)
...
def _add_gamecube_disc_metadata(
	rom: FileROM, game_info: GameInfo, header: bytes, tgc_data: 'Mapping[str, int] | None' = None
) -> None:
