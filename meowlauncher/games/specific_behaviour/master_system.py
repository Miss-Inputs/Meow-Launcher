	...
	if 0 < author_offset < 0xFFFF:
		...
		with contextlib.suppress(UnicodeDecodeError):
			metadata.developer = metadata.publisher = (
				rom.read(seek_to=author_offset, amount=255).partition(b'\x00')[0].decode('ascii')
			)
	if 0 < name_offset < 0xFFFF:
		with contextlib.suppress(UnicodeDecodeError):
			metadata.add_alternate_name(
				rom.read(seek_to=name_offset, amount=255).partition(b'\x00')[0].decode('ascii'),
				'Header Title',
			)
	if 0 < description_offset < 0xFFFF:
		with contextlib.suppress(UnicodeDecodeError):
			metadata.descriptions['Description'] = (
				rom.read(seek_to=description_offset, amount=255)
				.partition(b'\x00')[0]
				.decode('ascii')
			)


class BadSMSHeaderError(Exception):
...
def _parse_standard_header(rom: 'FileROM', base_offset: int) -> 'Mapping[str, Any]':
	...
	if header[:8] != b'TMR SEGA':
		raise BadSMSHeaderError('TMR missing')
	...
	product_code = (
		f'{"" if product_code_lo == 0 else product_code_lo}{_decode_bcd_multi(product_code_hi):04}'
	)
	...
	if region_code in regions:
		...
		raise BadSMSHeaderError('Weird region: ' + str(region_code))
...
def add_info_from_standard_header(rom: 'FileROM', metadata: 'GameInfo') -> None:
	...
	for possible_offset in possible_offsets:
		...
		try:
			...
			break
		except BadSMSHeaderError:
			continue

	if header_data:
		metadata.specific_info['Revision'] = header_data['Revision']
		metadata.product_code = header_data['Product code']
		metadata.specific_info['Region Code'] = header_data[
			'Region'
		]  # Too lazy to make an enum for both SMS and GG regions
...
def add_sms_gg_software_list_info(software: 'Software', metadata: 'GameInfo') -> None:
	...
	metadata.save_type = (
		SaveType.Cart if software.get_part_feature('battery') == 'yes' else SaveType.Nothing
	)
