...
def add_colecovision_software_info(software: 'Software', game_info: 'GameInfo') -> None:
	...
	normal_controller = input_info.CombinedController(
		[normal_controller_part, normal_controller_keypad]
	)

	super_action_controller_buttons = input_info.NormalController()
	super_action_controller_buttons.face_buttons = (
		4  # Not really on the face, they're on the hand grip part, but still
	)
	super_action_controller_buttons.dpads = 1
	super_action_controller_speed_roller = (
		input_info.Dial()
	...
	super_action_controller = input_info.CombinedController(
		[
			super_action_controller_buttons,
			super_action_controller_speed_roller,
...
def _get_uapce_games() -> 'Collection[Machine]':
	...
	if not mame.is_available:
		return []
	return frozenset(iter_machines_from_source_file('uapce', mame))
