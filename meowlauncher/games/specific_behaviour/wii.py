...
def _parse_tmd(metadata: 'GameInfo', tmd: bytes) -> None:
	...
	try:
		...
		if product_code:
			with contextlib.suppress(ValueError):
				metadata.specific_info['Virtual Console Platform'] = WiiVirtualConsolePlatform(
					product_code[0]
				)
...
def _parse_opening_bnr(metadata: 'GameInfo', opening_bnr: bytes) -> None:
	...
	if region_code == NintendoDiscRegion.NTSC_J:
		...
		NintendoDiscRegion.NTSC_U,
		NintendoDiscRegion.PAL,
		NintendoDiscRegion.RegionFree,
	}:
...
def _add_wii_disc_metadata(rom: FileROM, game_info: 'GameInfo') -> None:
	...
	if common_key:
		if game_partition_offset and have_pycrypto:
			...
			chunk_offset = (
				game_partition_offset + data_offset
			)  # + (index * 0x8000) but we only need 1st chunk (0x7c00 bytes of encrypted data each chunk)
...
def add_wii_custom_info(game: 'ROMGame') -> None:
	if game.rom.extension in {'gcz', 'iso', 'wbfs', 'gcm'}:
		...
		if (
			game.rom.name.lower() == 'boot'
		):  # Shouldn't happen I guess if homebrew detection works correctly but sometimes it be like that
