...
def _add_meta_xml_metadata(metadata: 'GameInfo', meta_xml: ElementTree.ElementTree) -> None:
	...
	if product_code:
		...
		with contextlib.suppress(ValueError):
			metadata.specific_info['Virtual Console Platform'] = WiiUVirtualConsolePlatform(
	...
	if mastering_date_text:
		try:
			...
			mastering_date = Date(
				mastering_datetime.year, mastering_datetime.month, mastering_datetime.day
			)
			metadata.specific_info['Mastering Date'] = mastering_date
			guessed_date = Date(mastering_date.year, mastering_date.month, mastering_date.day, is_guessed=True)
	...
	metadata.specific_info['Common Save Size'] = ByteAmount(
		meta_xml.findtext('common_save_size') or '0', 16
	)
	metadata.specific_info['Account Save Size'] = ByteAmount(
		meta_xml.findtext('account_save_size') or '0', 16
	)
	...
	rating_tags = {
		tag: int(tag.text) for tag in meta_xml.iter() if tag.tag.startswith('pc_') and tag.text
	}
	ratings = {
		tag.tag: rating & 0b0001_1111
		for tag, rating in rating_tags.items()
		if (rating & 0b1000_0000) == 0 and (rating & 0b0100_0000) == 0
	}
	if ratings:
		try:
...
def _add_homebrew_meta_xml_metadata(
	rom: ROM, metadata: 'GameInfo', meta_xml: ElementTree.ElementTree
) -> None:
	...
	if release_date_text:
		metadata.release_date = Date(
			release_date_text[0:4], release_date_text[4:6], release_date_text[6:8]
		)
	...
	metadata.specific_info['Homebrew Category'] = (
		meta_xml.findtext('category') or 'None'
	)  # Makes me wonder if it's feasible to include an option to get categories not from folders…
...
def _add_rpx_metadata(rom: ROM, metadata: 'GameInfo') -> None:
	...
	try:
		...
		_add_homebrew_meta_xml_metadata(
			rom, metadata, ElementTree.parse(rom.path.with_name('meta.xml'))
		)
...
def add_folder_metadata(rom: FolderROM, metadata: 'GameInfo') -> None:
	...
	assert (
		content_dir and meta_dir
	), 'It should be impossible for content_dir or meta_dir to be none, otherwise this would not have even been detected as a folder'
	...
	if content_dir.joinpath('assets').is_dir() and all(
		content_dir.joinpath('app', file).is_dir()
		for file in ('appinfo.xml', 'config.xml', 'index.html')
	):
	...
	boot_drc_path = meta_dir.joinpath(
		'bootDrcTex.tga'
	)  # Image displayed on the gamepad while loading
	if boot_drc_path.is_file():
		metadata.images['Gamepad Boot Image'] = boot_drc_path
	boot_tv_path = meta_dir.joinpath(
		'bootTvTex.tga'
	)  # Generally just bootDrcTex but higher resolution (and for the TV)
	...
	if (
		metadata.specific_info.get('Virtual Console Platform')
		== WiiUVirtualConsolePlatform.GBAOrPCEngine
	):
		metadata.specific_info['Virtual Console Platform'] = (
			WiiUVirtualConsolePlatform.GBA
			if rom.name == 'm2engage'
			else WiiUVirtualConsolePlatform.PCEngine
		)
