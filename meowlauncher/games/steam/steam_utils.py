...
from meowlauncher.data.name_cleanup.steam_developer_overrides import developer_overrides
...
def translate_language_list(languages: Mapping[bytes, Any]) -> Collection[Language]:
	langs = set()
	for language_name_bytes in languages:
		...
		if (
			language_name == 'koreana'
		):  # I don't know what the a at the end is for, but Steam does that
