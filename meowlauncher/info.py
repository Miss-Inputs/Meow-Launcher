...
class GameInfo:
	...
	def to_launcher_fields(self) -> 'MutableMapping[str, MutableMapping[str, Any]]':
		...
		info_fields: 'MutableMapping[str, Any]' = {
			...
			'Save Type': 'Memory Card'
			if self.save_type == SaveType.MemoryCard
			else self.save_type.name,
			'Publisher': self.publisher,
			'Developer': self.developer,
			'Product Code': self.product_code,
			'Regions': tuple(region.name for region in self.regions),
			'Media Type': (
				'Optical Disc' if self.media_type == MediaType.OpticalDisc else self.media_type.name
			)
			if self.media_type
			else None,
			'Notes': self.notes,
		...
		if self.release_date and self.release_date.year:
			info_fields['Year'] = (
				...
				if self.release_date.is_guessed
				else self.release_date.year
			)
