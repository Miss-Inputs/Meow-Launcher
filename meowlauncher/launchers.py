...
class LaunchParams():
	...
	def wrap(self, command: str):
		return LaunchParams(command, [self.exe_name] + self.exe_args)

	def prepend_command(self, prepended_command):
		return MultiCommandLaunchParams([prepended_command], self, [])
		
	def append_command(self, appended_params):
		return MultiCommandLaunchParams([], self, [appended_params])

	def replace_path_argument(self, path: str):
