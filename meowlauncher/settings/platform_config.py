...
from ._config_utils import parse_config_section_value, parse_path_list, parse_string_list
...
def _get_config(section: 'configparser.SectionProxy', platform_name: str) -> PlatformConfig:
	...
	chosen_emulators = tuple(
		f'{chosen_emulator} ({platform_name})'
		if chosen_emulator in {'MAME', 'Mednafen', 'VICE'}
		else chosen_emulator
...
class PlatformConfigs:
	...
	def __init__(self) -> None:
		...
		self.configs = {
			platform_name: _get_config(section, platform_name)
			for platform_name, section in parser.items()
		}
