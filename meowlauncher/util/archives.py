...
def subprocess_sevenzip_list(path: 'Path') -> 'Iterator[FilenameWithMaybeSizeAndCRC]':
	...
	"""
	proc = subprocess.run(
		['7z', 'l', '-slt', '--', path], capture_output=True, text=True, check=False
	)
...
def subprocess_sevenzip_get(
	path: 'Path', filename: str, offset: int = 0, amount: int = -1
) -> bytes:
	with subprocess.Popen(
		['7z', 'e', '-so', '--', path, filename], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL
	) as proc:
...
def subprocess_sevenzip_getsize(path: 'Path', filename: str) -> ByteAmount:
	proc = subprocess.run(
		['7z', 'l', '-slt', '--', path, filename], capture_output=True, text=True, check=False
	)
	if proc.returncode != 0:
		raise BadSubprocessedArchiveError(f'{path}: {proc.returncode} {proc.stdout} {proc.stderr}')
	...
	for line in proc.stdout.splitlines():
		if line.startswith('------'):
			...
			continue
		if found_file_line and (fullmatch := _sevenzip_size_reg.fullmatch(line)):
...
def zip_get(path: 'Path', filename: str, offset: int = 0, amount: int = -1) -> bytes:
	with zipfile.ZipFile(path, 'r') as zip_file, zip_file.open(filename, 'r') as file:
		if offset:
			file.seek(offset)
...
def libarchive_get(path: 'Path', filename: str, offset: int = 0, amount: int = -1) -> bytes:
	with path.open('rb') as f, libarchive.Archive(f, 'r') as a:
		for item in a:
...
def compressed_list(path: 'Path') -> 'Iterator[FilenameWithMaybeSizeAndCRC]':
	...
	raise NotImplementedError(
		'You have nothing to read', path, 'with, try installing 7z or py7zr or python-libarchive'
	)
