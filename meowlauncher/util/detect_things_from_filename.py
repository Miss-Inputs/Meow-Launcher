...
_nointro_language_list_regex = re.compile(
	...
	return list(
		itertools.chain.from_iterable(
			[
				...
				if language.english_name in tag or language.native_name in tag
			]
			for tag in tags
		)
	)
...
def get_nointro_language_list_from_filename_tags(
	tags: Sequence[str],
) -> Sequence[region_info.Language] | None:
	for tag in tags:
...
def get_tosec_languages_from_filename_tags(
	tags: Sequence[str],
) -> Sequence[region_info.Language] | None:
	...
	for tag in tags:
		if not found_year_tag and _tosec_date_tag_regex.match(tag):
			found_year_tag = True
			continue
...
def get_regions_from_filename_tags_strictly(
	tags: Sequence[str],
) -> Sequence[region_info.Region] | None:
...
def get_regions_from_filename_tags_loosely(tags: Sequence[str]) -> Sequence[region_info.Region]:
	regions: list[region_info.Region] = []
	for tag in tags:
		regions.extend(region for region in region_info.regions if region.name in tag)
...
def get_tosec_region_list_from_filename_tags(
	tags: Sequence[str],
) -> Sequence[region_info.Region] | None:
	...
	for tag in tags:
		...
		try:
			regions = tuple(
				region_info.regions_by_short_code[component] for component in components
			)
		except KeyError:
			# This wasn't the tag we wanted
			continue
...
def get_regions_from_filename_tags(
	tags: Sequence[str], loose: bool = False
) -> Sequence[region_info.Region] | None:
...
_date_regex = re.compile(
	r'\((?P<year>[x\d]{4})\)|\((?P<year2>\d{4})-(?P<month>\d{2})-(?P<day>\d{2})\)|\((?P<day2>\d{2})\.(?P<month2>\d{2})\.(?P<year3>\d{4})\)'
)
...
_version_regex = re.compile(
	r'\(([vV]\d+(?:\.\w+)?)\)'
)  # Very loose match, I know, but sometimes versions have stuff on the end like v1.2b or whatever and I don't wanna overcomplicate things
_version_number_regex = re.compile(
	r'\((?:version|ver|ver\.)\s+([\d.]+)[^)]*\)'
)  # This one is a bit more specific and shows up in MAME machine names sometimes
